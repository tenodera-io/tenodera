...
- **A few admin subsystems and all reads run as root without dropping**: SSH access management, the Security page (fail2ban/SELinux/AppArmor), host enrollment/tokens, and read-only system introspection execute at the agent's root privilege after only the `require_admin()`/authentication check. For the admin subsystems the injected admin role is the boundary — see the Authorization section and `THREAT_MODEL.md` §6.
