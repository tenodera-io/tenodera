...
- **Reads are being brokered per-user, incrementally.** The journal, log files under `/var/log` (tail/search/filter), the process list (`ps`, honouring `hidepid`), and the process owning each listening socket (`ss -p`) now run *as the logged-in user* — their own file/group permissions decide, and the superuser password escalates via `sudo` as that user, exactly like writes. The remaining privileged reads — container listing/inspect/logs — still run at the agent's privilege (root), as does the baseline world-readable introspection (metrics, disk, network stats, hardware/system info, DNS, packages) by design; for those, any authenticated user can read that state regardless of their rights on the host — see `THREAT_MODEL.md` §6
...
- **A few admin subsystems, and the not-yet-brokered reads, run as root without dropping**: SSH access management, the Security page (fail2ban/SELinux/AppArmor), host enrollment/tokens, container introspection, and the baseline world-readable system introspection execute at the agent's root privilege after only the `require_admin()`/authentication check. (The journal, log files, process list, and listening-port owners are now brokered to the logged-in user and drop privilege like writes.) For the admin subsystems the injected admin role is the boundary — see the Authorization section and `THREAT_MODEL.md` §6.
