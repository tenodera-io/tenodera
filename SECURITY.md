...
- **Every privileged read is brokered per-user.** The journal, log files under `/var/log` (tail/search/filter), the process list (`ps`, honouring `hidepid`), the process owning each listening socket (`ss -p`), container reads (list/inspect/logs/stats/volumes/networks), user crontabs, kdump crash-dump content, and the certificate listing all run *as the logged-in user* — their own file/group permissions decide, and the superuser password escalates via `sudo` as that user, exactly like writes. What still runs at the agent's privilege (root) is only the baseline **world-readable** introspection (metrics, disk, network stats, hardware/system info, DNS, packages, systemd timers, system cron files, public cert metadata) — by design, since it exposes nothing a normal user couldn't already read. See `THREAT_MODEL.md` §6
...
- **A few admin subsystems, and the baseline world-readable introspection, run as root without dropping**: SSH access management, the Security page (fail2ban/SELinux/AppArmor), host enrollment/tokens, and the world-readable system introspection (metrics, disk, DNS, packages, system cron files, public cert metadata) execute at the agent's root privilege after only the `require_admin()`/authentication check. (Every *privileged* read — journal, log files, process list, listening-port owners, container reads, user crontabs, kdump crash-dump content — is brokered to the logged-in user and drops privilege like writes.) For the admin subsystems the injected admin role is the boundary — see the Authorization section and `THREAT_MODEL.md` §6.
