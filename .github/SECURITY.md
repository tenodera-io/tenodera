...
- **The gateway is loopback-only; a Caddy reverse proxy fronts it by default.** The installer ships `/etc/tenodera/tenodera.cnf` with `TENODERA_BIND_ADDR=127.0.0.1` and `TENODERA_ALLOW_UNENCRYPTED=1` — the gateway itself runs plain HTTP reachable **only from the panel host** — and then installs [Caddy](https://caddyserver.com) with a generated `/etc/caddy/Caddyfile` that terminates **HTTPS on the network** and reverse-proxies to that loopback backend. Out of the box Caddy uses a self-signed internal-CA cert (swap in a domain + real cert for production). If Caddy isn't set up (no network / unsupported distro), reach the panel over an SSH tunnel (`ssh -L 9090:127.0.0.1:9090 <host>`, then `http://localhost:9090`)
- **Don't bypass the proxy without TLS.** The loopback bind means the plain-HTTP backend is reachable only via Caddy. If you instead bind `0.0.0.0`, **first** configure TLS (`TENODERA_TLS_CERT`/`TENODERA_TLS_KEY`) and remove `TENODERA_ALLOW_UNENCRYPTED=1`, or you will serve the unencrypted panel on every interface
...
- **Use a real certificate in production** — the installer's Caddy default is a self-signed internal-CA cert (browser warning, agents run with `--insecure`); put a domain + CA-signed certificate in the Caddyfile for production and drop the agents' insecure flag
