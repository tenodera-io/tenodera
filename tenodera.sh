...
if [ "${1:-}" = "--uninstall" ]; then
  ...
  # Reverse proxy config we generated (identified by its marker). Caddy itself is
  # left installed — it may serve other sites.
  if [ -f /etc/caddy/Caddyfile ] && grep -q 'generated by the Tenodera installer' /etc/caddy/Caddyfile 2>/dev/null; then
    rm -f /etc/caddy/Caddyfile
    systemctl reload caddy 2>/dev/null || systemctl stop caddy 2>/dev/null || true
    info "Removed the generated /etc/caddy/Caddyfile (Caddy itself left installed)"
  fi

  # Service account created at install time.
