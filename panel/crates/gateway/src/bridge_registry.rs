...
/// Strip the `prefix-` from a channel ID in a message.
/// Returns `None` if the channel does not start with this prefix.
pub fn strip_prefix(msg: Message, prefix: &str) -> Option<(Message, String)> {
    let pfx = format!("{prefix}-");

    let channel_str = match &msg {
        Message::Data { channel, .. } => channel.as_str().to_string(),
        Message::Control { channel, .. } => channel.as_str().to_string(),
        Message::Close { channel, .. } => channel.as_str().to_string(),
        Message::Ready { channel } => channel.as_str().to_string(),
        _ => return None,
    };

    if !channel_str.starts_with(&pfx) {
        return None;
    }

    let original = &channel_str[pfx.len()..];
    if original.is_empty() {
        return None;
    }
    ...
    let extracted_prefix = prefix.to_string();
    Some((stripped, extracted_prefix))
}
